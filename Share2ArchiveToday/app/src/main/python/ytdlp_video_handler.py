...
class VideoHandler:
    ...
    def download_video_with_strategies(
        ...
        video_format = format_processor.get_quality_based_format(quality)
    ...
    def download_dash_video(
        ...
        try:
            ...
            dash_strategies = ['best']
            
            for strategy in dash_strategies:
                if self.cancelled:
                    return create_result_dict(success=False, error='Download cancelled by user')
                
                try:
                    debug_print(f"[info] Trying DASH strategy: {strategy}", flush=True)
            ...
            debug_print(f"[info] DASH strategies failed, falling back to general approach", flush=True)
